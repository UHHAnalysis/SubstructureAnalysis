...
#include "include/SubstructureCycleNew.h"
#include "include/SubstructureHistsNew.h"
...
void SubstructureCycleNew::begin_dataset(Context & ctx){
    // 1. set up selections:
    ...
    // 2. Set up Histograms:
    h_nocuts.reset(new SubstructureHistsNew(ctx, "NoCuts"));
    h_btag.reset(new SubstructureHistsNew(ctx, "BTag"));
    h_nobtag.reset(new SubstructureHistsNew(ctx, "NoBTag"));
    h_topsel.reset(new SubstructureHistsNew(ctx, "TopSel"));
    ...
    // this cycle on an output file containing these branches, e.g. one previously produced with SubstructureCycleNew ...
    read_example_data = string2bool(ctx.get_setting("readSubstructureData"));
...
void SubstructureCycleNew::process(EventCalc & event, Context & ctx) {
    ...
    if(read_example_data){
        cout << "Substructure data read: int = " << event_intdata_in << "; jet pt = " << event_jetdata_in.pt() << endl;
    }
...
// as we want to run the SubstructureCycleNew directly with AnalysisModuleRunner, we have to make sure we are found by the class name.
// This is achieved by calling this macro:
REGISTER_ANALYSIS_MODULE(SubstructureCycleNew)
