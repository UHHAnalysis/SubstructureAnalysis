...
void SubstructureSelectionCycle::BeginInputData( const SInputData& id ) throw( SError )
{
  ...
  second_selection->addSelectionModule(new NTopJetSelection(1,int_infinity(),150,2.5)); //leading jet with pt>150 GeV
  ...
  Jetselveto_150->addSelectionModule(new NCAGenJetveto(100,2,2.5));
...
void SubstructureSelectionCycle::ExecuteEvent( const SInputData& id, Double_t weight) throw( SError )
{
  ...
  if(!preselection->passSelection() && !gensel && !m_gen_presel_hadron) throw SError( SError::SkipEvent );
  ...
  // control histograms
  if(preselection->passSelection()) FillControlHists("_Presel");
  ...
  // control histograms
  if(preselection->passSelection()) FillControlHists("_Cleaned");
  ...
  if(!first_selection->passSelection() && !gensel && !m_gen_presel_hadron) throw SError( SError::SkipEvent );
  ...
  if(!second_selection->passSelection() && !gensel && !m_gen_presel_hadron) throw SError( SError::SkipEvent );

  if(!m_reversed_electron_selection) {
    if(!trangularcut_selection->passSelection() && !gensel && !m_gen_presel_hadron) throw SError( SError::SkipEvent );
  } else {
    if(!trangularcut_selection->passInvertedSelection()  && !gensel  && !m_gen_presel_hadron) throw SError( SError::SkipEvent );
  }
